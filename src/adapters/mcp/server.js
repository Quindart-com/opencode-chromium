...
function mcpImageContent(result, runtime) {
  try {
    const images = [];
    for (const candidate of imageCandidates(result)) {
      if (images.length >= IMAGE_ATTACHMENT_LIMIT) break;
      if (typeof candidate.base64 === "string") {
        images.push({ type: "image", data: candidate.base64, mimeType: candidate.mimeType });
      } else if (runtime?.artifacts) {
        const artifact = runtime.artifacts.read(candidate.artifactId ?? candidate.uri);
        if (artifact?.data) images.push({ type: "image", data: Buffer.from(artifact.data).toString("base64"), mimeType: candidate.mimeType });
      }
    }
    return images;
  } catch {
    // Image content is an enhancement; never fail the tool result for it.
  }
  return [];
}
...
function sanitizeImagePayloads(value, depth = 0) {
  ...
  if (inline) {
    ...
    return { ...rest, base64Bytes: Math.floor(base64.replace(/=+$/, "").length * 0.75), imageDelivery: "content" };
  }
  const output = {};
...
export { mcpImageContent, sanitizeImagePayloads };
  ...
  for (const registry of registries) {
    for (const [name, definition] of Object.entries(registry)) {
      ...
      try {
        ...
        const images = mcpImageContent(result, runtime);
        return {
          content: [{ type: "text", text: conciseText(result) }, ...images],
